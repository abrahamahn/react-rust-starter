...
pub fn router(database: Database, access: Service) -> Router {
    Router::new()
        ...
        .route("/api/notes/{id}", put(update).delete(delete))
        .with_state(StateData { database, access })
}
async fn list(State(s): State<StateData>, headers: HeaderMap) -> Result<Json<Vec<Note>>, Error> {
    ...
    let mut client = s.database.client().await?;
    let tx = client.transaction().await?;
    let session = authorize(&tx, &digest, true).await?;
    let result = store::list(&tx, &session.user.id).await?;
    tx.commit().await?;
    Ok(Json(result))
}
async fn create(
    ...
    headers: HeaderMap,
    input: Result<Json<WriteNote>, JsonRejection>,
    ...
    let tx = client.transaction().await?;
    let session = authorize(&tx, &digest, true).await?;
    let result = store::create(&tx, &session.user.id, &input).await?;
    tx.commit().await?;
...
async fn update(
    ...
    let tx = client.transaction().await?;
    let session = authorize(&tx, &digest, true).await?;
    let result = store::update(&tx, &session.user.id, &id, &input).await?;
    tx.commit().await?;
    Ok(Json(result))
}
async fn delete(
    ...
    Path(id): Path<String>,
    headers: HeaderMap,
    input: Result<Json<DeleteNote>, JsonRejection>,
    ...
    let tx = client.transaction().await?;
    let session = authorize(&tx, &digest, true).await?;
    store::delete(&tx, &session.user.id, &id, input.revision).await?;
    tx.commit().await?;
