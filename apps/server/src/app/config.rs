...
impl Config {
    ...
    pub fn from_lookup(get: impl Fn(&str) -> Option<String>) -> Result<Self, Error> {
        ...
        let bind = get("APP_BIND")
            ...
            .parse::<SocketAddr>()
            .map_err(|_| Error::Config("invalid APP_BIND"))?;
        if !bind.ip().is_loopback() || bind.port() == 0 {
            return Err(Error::Config(
                "bind must be loopback; use the documented HTTPS reverse proxy",
            ));
        }
        ...
        if production {
            if parsed.scheme() != "https" {
                return Err(Error::Config("production requires HTTPS APP_ORIGIN"));
            }
        ...
        {
            return Err(Error::Config(
                "development origin must be a loopback HTTP IP",
            ));
        }
        let dsn = get("APP_DATABASE_URL").ok_or(Error::Config("APP_DATABASE_URL is required"))?;
        let mut database = dsn
            .parse::<tokio_postgres::Config>()
            .map_err(|_| Error::Config("invalid database configuration"))?;
        let local = database.get_hosts().len() == 1
            && match &database.get_hosts()[0] {
                Host::Tcp(host) => host.parse::<IpAddr>().is_ok_and(|ip| ip.is_loopback()),
                _ => false,
            };
        if !local
            || !database.get_hostaddrs().is_empty()
            || database.get_options().is_some()
            || database.get_ssl_mode() != SslMode::Disable
            || database.get_user().is_none()
        {
            return Err(Error::Config(
                "database requires explicit loopback IP, user and sslmode=disable; no overrides",
            ));
        }
        if !database
            .get_dbname()
            .is_some_and(|name| name.starts_with("starter_") && name.len() > 8)
        {
            return Err(Error::Config(
                "use a separate database named starter_<name>",
            ));
        }
        ...
        let smtp_port = get("SMTP_PORT")
            .unwrap_or_else(|| if production { "587" } else { "1025" }.into())
            .parse::<u16>()
            .map_err(|_| Error::Config("invalid SMTP_PORT"))?;
        ...
        if production {
            if smtp_user.is_none() || get("MAIL_FROM").is_none() || get("SMTP_HOST").is_none() {
                return Err(Error::Config(
                    "production requires explicit SMTP host, credentials and MAIL_FROM",
                ));
            }
...
mod tests {
    ...
    fn config(overrides: &[(&str, &str)]) -> Result<Config, Error> {
        Config::from_lookup(|key| {
            overrides
                ...
                .map(|(_, v)| (*v).into())
                .or_else(|| {
                    (key == "APP_DATABASE_URL").then(|| {
                        "postgresql://tester:local-test@127.0.0.1/starter_test?sslmode=disable"
                            .into()
                    })
                })
        })
    ...
    #[test]
    fn accepts_local_and_hides_secrets() {
        assert_eq!(config(&[]).unwrap().bind.port(), 8088);
        let error = config(&[("APP_DATABASE_URL", "private-secret-invalid")])
            .err()
            .unwrap();
    ...
    fn rejects_unsafe_configuration() {
        for values in [
            vec![("APP_BIND", "0.0.0.0:8088")],
            vec![("APP_MODE", "production")],
            vec![("APP_ORIGIN", "http://127.0.0.1:5178/path")],
            vec![("APP_ORIGIN", "http://127.0.0.1:5178#token")],
            vec![(
                "APP_DATABASE_URL",
                "host=127.0.0.1 hostaddr=10.0.0.1 user=u dbname=starter_test sslmode=disable",
            )],
