...
pub(super) fn hash_password(password: &str) -> Result<String, Error> {
    ...
    argon2()?.hash_password(password.as_bytes(), &salt).map(|hash| hash.to_string()).map_err(|_| Error::Internal)
}
pub(super) fn verify(password: &str, encoded: Option<&str>) -> Result<bool, Error> {
    match encoded {
        ...
        None => { let _ = hash_password(password)?; Ok(false) }
    }
}
...
pub(super) fn digest(value: &str) -> String { format!("{:x}", Sha256::digest(value.as_bytes())) }
#[cfg(test)]
mod tests {
    use super::*;
    #[test] fn passwords_are_salted_and_verified() {
    ...
    #[test] fn tokens_are_random_and_stored_by_digest() {
