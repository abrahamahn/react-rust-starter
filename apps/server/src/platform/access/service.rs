...
struct Throttle {
    ...
    accounts: HashMap<String, (Instant, usize)>,
}
impl Throttle {
    fn admit(&mut self, account: &str, now: Instant) -> Result<(), Error> {
        ...
        self.accounts
            .retain(|_, (at, _)| now.duration_since(*at) < Duration::from_secs(300));
        let key = crypto::digest(account);
        if !self.accounts.contains_key(&key) && self.accounts.len() >= 512 {
            return Err(Error::RateLimited);
        }
...
impl Service {
    ...
    async fn verify(&self, password: String, hash: Option<String>) -> Result<(), Error> {
        ...
        let valid = tokio::task::spawn_blocking(move || {
            let _permit = permit;
            crypto::verify(&password, hash.as_deref())
        })
        .await
        .map_err(|_| Error::Internal)??;
        if !valid {
    ...
    pub(super) async fn register(
        &self,
        input: Credentials,
    ) -> Result<(SessionView, String, bool), Error> {
    ...
    pub(super) async fn login(
        ...
        let input = input.validate(false)?;
        self.admit(&input.email)?;
        let record = self.store.password(&input.email).await?;
        self.verify(input.password, record.as_ref().map(|r| r.hash.clone()))
            .await?;
    ...
    pub(super) async fn request_link(
        ...
        if self
            .store
            .issue(&email, purpose, &crypto::digest(&token))
            .await?
        {
            mail.send(self.link(email, &token, purpose));
        }
    ...
    pub(super) async fn confirm(&self, input: TokenRequest) -> Result<(), Error> {
        ...
        self.store
            .consume(&crypto::digest(&input.token), "verify", None)
            .await?;
        Ok(())
    }
    pub(super) async fn reset(&self, input: ResetRequest) -> Result<(), Error> {
        validate_token(&input.token)?;
        validate_password(&input.password, true)?;
        self.admit(&input.token)?;
        let mail = self.mailer.reserve()?;
        let hash = self.hash(input.password).await?;
        let email = self
            .store
            .consume(&crypto::digest(&input.token), "reset", Some(&hash))
            .await?;
    ...
    pub(super) async fn change(&self, digest: &str, input: ChangePassword) -> Result<(), Error> {
        validate_password(&input.current_password, false)?;
        validate_password(&input.password, true)?;
        let view = self.store.current(digest).await?;
        self.admit(&view.user.email)?;
        let record = self
            .store
            .password(&view.user.email)
            .await?
            .ok_or(Error::Unauthenticated)?;
        self.verify(input.current_password, Some(record.hash.clone()))
            .await?;
        let mail = self.mailer.reserve()?;
        let hash = self.hash(input.password).await?;
        ...
        mail.send(Delivery {
            to: view.user.email,
            subject: "Password changed".into(),
            text: "Your password was changed. All previous sessions were revoked.".into(),
        });
        Ok(())
    }
    pub fn session_digest(&self, headers: &axum::http::HeaderMap) -> Result<String, Error> {
        self.session_token(headers)
            .map(|s| crypto::digest(&s))
            .ok_or(Error::Unauthenticated)
    }
    pub(super) fn session_token(&self, headers: &axum::http::HeaderMap) -> Option<String> {
        ...
        if tokens.next().is_some() || validate_token(first).is_err() {
            return None;
        }
        Some(first.into())
    }
    pub fn check_origin(&self, headers: &axum::http::HeaderMap) -> Result<(), Error> {
        if headers.get_all("origin").iter().count() != 1
            || headers.get("origin").and_then(|h| h.to_str().ok())
                != Some(self.config.origin.as_str())
            || headers
                .get("x-starter-client")
...
mod tests {
    ...
    fn throttle_is_bounded_and_expires() {
        ...
        for _ in 0..10 {
            throttle.admit("user@example.test", start).unwrap();
        }
        assert!(matches!(
            throttle.admit("user@example.test", start),
            Err(Error::RateLimited)
        ));
        throttle
            .admit("user@example.test", start + Duration::from_secs(301))
            .unwrap();
