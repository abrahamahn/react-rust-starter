...
const MIGRATIONS: &[(&str, &str)] = &[(
    "0001_starter",
    include_str!("../../../migrations/0001_starter.sql"),
)];
#[derive(Clone)]
pub struct Database {
    pool: Pool,
...
impl Database {
    ...
    pub async fn client(&self) -> Result<Object, Error> {
        self.pool.get().await.map_err(|_| Error::Unavailable)
    }
    pub async fn ready(&self) -> Result<(), Error> {
        let client = self.client().await?;
        let exists: bool = client
            .query_one(
                "SELECT to_regclass('starter.schema_migrations') IS NOT NULL",
                &[],
            )
            .await?
            .get(0);
        ...
        let rows = client
            .query(
                "SELECT version,name,checksum FROM starter.schema_migrations ORDER BY version",
                &[],
            )
            .await?;
        validate(&rows)?;
    ...
    pub async fn migrate(&self) -> Result<(), Error> {
        ...
        for (index, (name, sql)) in MIGRATIONS.iter().enumerate().skip(applied) {
            ...
            tx.execute(
                "INSERT INTO starter.schema_migrations(version,name,checksum) VALUES($1,$2,$3)",
                &[&((index + 1) as i64), name, &checksum(sql)],
            )
            .await?;
        }
        tx.commit().await?;
...
fn validate(rows: &[Row]) -> Result<(), Error> {
    ...
    for (index, row) in rows.iter().enumerate() {
        ...
        if version != (index + 1) as i64
            || name != MIGRATIONS[index].0
            || stored != checksum(MIGRATIONS[index].1)
        {
            return Err(Error::MigrationDrift);
        }
    }
